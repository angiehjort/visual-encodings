// https://github.com/vizabi/@vizabi/shared-components#readme v1.59.0 build 1777717344906 Copyright 2026 Gapminder Foundation and contributors
(function (global, factory) {
...
})(this, (function (exports, d3, mobx) { 'use strict';
    ...
    addReaction(method, {ignoreStatus = false, throttle_ms = null} = {}){
      ...
      if(!this.reactions.has(method)){
        this.reactions.set(method, 
          mobx.autorun(() => {
            const run = throttle_ms ? throttle(method.bind(this), throttle_ms) : method.bind(this);
  ...
  class BrushSlider extends BaseComponent {
    ...
    setup(_options) {
      ...
      const options = this.options = deepExtend(deepExtend({}, OPTIONS$7), _options || {});
  ...
  const PROFILE_CONSTANTS_FOR_PROJECTOR$5 = () => ({
    MEDIUM: {
      infoElHeight: 26
    },
    LARGE: {
  ...
  class _DataWarning extends BaseComponent {
    ...
    _updateLayoutProfile(){
      ...
      this.profileConstants = this.services.layout.getProfileConstants(
        PROFILE_CONSTANTS$5(this.width, this.height), 
        PROFILE_CONSTANTS_FOR_PROJECTOR$5(this.width, this.height)
      );
  ...
  const decorated$g = mobx.decorate(DateTimeBackground, {
    "MDL": mobx.computed,
    ...
    _interact() {
      ...
      return {
        ...
        clickToAddAllinGroup(d) {
          ...
          _this.model.data.filter.addUsingLimitedStructure({key: d[KEY$5], dim, prop});
        },
        clickToRemoveAllinGroup(d) {
          if (!isEntityConcept(_this.MDL.color.data.conceptProps)) return;
          const dim = _this.model.encoding.color.data.space[0];
          const prop = _this.MDL.color.data.concept;
          _this.model.data.filter.deleteUsingLimitedStructure({key: d[KEY$5], dim, prop});
        },
        clickToRemoveAllinOtherGroups(d) {
          if (!isEntityConcept(_this.MDL.color.data.conceptProps)) return;
          const everythingElse = _this.MDL.legend.dataArray.map(m => m[KEY$5]).filter(f => f !== d[KEY$5]);
          const dim = _this.model.encoding.color.data.space[0];
          const prop = _this.MDL.color.data.concept;
        ...
        disableAddAll(d){
          ...
          const dim = _this.model.encoding.color.data.space[0];
          const prop = _this.MDL.color.data.concept;
          return !_this.model.data.filter.isAlreadyRemovedUsingLimitedStructure({key: d[KEY$5], dim, prop});
        },
        disableRemoveAll(d){
          if (!isEntityConcept(_this.MDL.color.data.conceptProps)) return true;
          if (_this.root.ui.dialogs?.markercontrols?.disableAddRemoveGroups) return true;
  ...
  const decorated$f = mobx.decorate(ColorLegend, {
    "MDL": mobx.computed
  });
  ...
  const PROFILE_CONSTANTS_FOR_PROJECTOR$4 = {
    SMALL: {},
    MEDIUM: {},
    LARGE: {}
  };
  ...
  class Dialog extends CollectionMixin(BaseComponent) {
    ...
    _updateLayoutProfile(){
      ...
      this.profileConstants = this.services.layout.getProfileConstants(PROFILE_CONSTANTS$4, PROFILE_CONSTANTS_FOR_PROJECTOR$4);
      this.height = this.element.node().clientHeight || 0;
      this.width = this.element.node().clientWidth || 0;
  ...
  const decorated$e = mobx.decorate(Dialog, {
    "MDL": mobx.computed
  });
  ...
  const decorated$d = mobx.decorate(LabelSizeHelper, {
    "MDL": mobx.computed,
    ...
    positionLabel(d, cache, context, duration, showhide, lineGroup, position) {
      ...
      if (lockPosition) {
        ...
        if (topCornerCase) {
          vPosNew++;
          const deltaX = resolvedX0 - cached.rectBBox.width;
          resolvedY = resolvedY0 + cached.rectBBox.height - offsetY * 0.5 + (deltaX > 0 && cached.scaledS0 > deltaX ? cathetus(cached.scaledS0, deltaX) : cached.scaledS0);
        }
      }
  ...
  class MarkerContextmenu extends BaseComponent {
    ...
    _transferSelectionOnExplode({dim, parentKey, targetProp}) {
      ...
      if (!selectedFilter.has({[KEY$4]: parentKey})) return;
      this._getChildKeys({dim, parentKey, targetProp}).then(childKeys => {
        mobx.runInAction(() => {
          childKeys.forEach(key => selectedFilter.set({[KEY$4]: key}));
          selectedFilter.delete({[KEY$4]: parentKey});
        });
      });
    }
    ...
    _deselectChildrenOnFold({dim, parentKey, childProp}) {
      this._getChildKeys({dim, parentKey, targetProp: childProp}).then(childKeys => {
        mobx.runInAction(() => {
          const selectedFilter = this._getSelectedFilter();
          childKeys.forEach(key => selectedFilter.delete({[KEY$4]: key}));
        });
      });
    }
    ...
    _interact() {
      ...
      return {
        ...
        clickToExplode(d) {
          ...
          _this._transferSelectionOnExplode({dim, parentKey: d[KEY$4], targetProp: explodeProp});
  ...
  function getMatchingSpace$1(spaces, targetSpace){
    return spaces.find(s => spacesAreEqual$3(s, targetSpace));
  }
  ...
  function removeDulicates$1(array){
    const result = [];
    array.forEach(space => {
      if(!result.some(s => spacesAreEqual$3(s, space)))
        result.push(space);
    });
    return result;
  }
  ...
  class _SpaceConfig extends BaseComponent {
    ...
    updateMarker(){
      ...
      options.enter().append("option")
        .text(d => d.join())
        .merge(options)
        .property("selected", d => spacesAreEqual$3(d, this.model.data.space));
    ...
    getSpaceCompatibilityStatus(encoding){
      ...
      if (spacesAreEqual$3(encoding.data.space, proposedSpace)) 
    ...
    updateApplyCancelButtons(){
      const hide = !this.proposedSpace || spacesAreEqual$3(this.proposedSpace, this.model.data.space);
  ...
  const decorated$a = mobx.decorate(SteppedSlider, {
    "MDL": mobx.computed
  });
  ...
  const decorated$9 = mobx.decorate(MinMaxInputs, {
    "MDL": mobx.computed
  });
  ...
  class DeepLeaf{
    ...
    updateSpaceSection(datumForDS = this._getDatumForDS()){
      ...
      if(this._isSelectedConcept() && (multipleSpacesAvailable() || !spacesAreEqual$2(datumForDS.spaces[0], markerSpace) && !shorterThanMarkerSpace())) {
    ...
    updateResetApplyButtons(datumForDS = this._getDatumForDS()) {
      ...
      spaceContainer.select(".vzb-treemenu-leaf-space-reset")
        .classed("vzb-hidden", spacesAreEqual$2(currentSpace, defaultSpace));
      spaceContainer.select(".vzb-treemenu-leaf-space-apply")
        .classed("vzb-hidden", !this.spaceChanged)
        .classed("vzb-disabled", !selectedFilter && !spacesAreEqual$2(currentSpace, selectedSpace));
    }
  ...
  class TreeMenu extends BaseComponent {
    ...
    getBestFittingDataSourceAndSpace(byDataSources) {
      ...
      const dsCandidates = byDataSources.filter(f => f.spaces.find(s => spacesAreEqual$1(s, bestSpace))).map(m => m.dataSource);
    ...
    getNearestSpaceToMarkerSpace(spaces){
      ...
      if (spaces.find(f => spacesAreEqual$1(f, this.model.data.space))) 
  ...
  class ZoomButtonList extends BaseComponent {
    ...
    updateHiding(){
      this.element.selectAll("button")
        .classed("vzb-hidden", d => d.hide && getProp(this, d.hide.split(".")) );
    }
  ...
  class SingleHandleSlider extends decorated$h {
  ...
  class BubbleSize extends decorated$h {
    setup(_options) {
...
`;
  ...
  class About extends decorated$e {
    constructor(config) {
      config.template = `
  ...
  class Find extends decorated$e {
    constructor(config) {
      config.template = `
      <div class='vzb-dialog-modal'>
  ...
  class SectionFind extends MarkerControlsSection {
      ...
      this.model.data.spaceCatalog.then(spaceCatalog => {
        ...
        if (spaceCatalog[dim].entities) {
          ...
          allowedEntities.forEach(entity => {
            if (explicitlyAddedEntities.includes(entity[KEY$3]) && !this.parent.marksFromAllFrames.some(s => s[dim] === entity[dim])) {
              const push = {
                [KEY$3]: entity[KEY$3],
                [dim]: entity[dim], 
                name: entity.name, 
    ...
    _createListItem(dataLength, colorFields, listItem) {
          ...
          if (colorFields && colorFields.includes(data.prop)) {
            view.append("span")
              ...
              .style("background-color", d => {
                const color = _this.colorScale(d[colorConcept] || d[KEY$3]);
                return color;
              });
          }
          view.append("span")
      ...
      listItem.append("span")
        ...
        .on("click", (event, d) => {
          ...
          this.model.data.filter.deleteUsingLimitedStructure({key: d[KEY$3], dim: primaryDimension, prop: primaryDimension, isness});
          this.parent._clearSearch();
          this.parent.updateSearch();
        });
    ...
    setModel = {
      select: (d) => {
        ...
        mobx.runInAction(() => _getLeafChildren(d).forEach(child => this.MDL.selected.data.filter.toggle(child)));
      },
      deselect: (d) => {
        mobx.runInAction(() => _getLeafChildren(d).forEach(child => this.MDL.selected.data.filter.delete(child)));
      },
      highlight: (d) => {
        if (d.missingDataForFrame || d.missingData) return;
        mobx.runInAction(() => _getLeafChildren(d).forEach(child => this.MDL.highlighted.data.filter.set(child)));
      },
      unhighlight: (d) => {
        mobx.runInAction(() => _getLeafChildren(d).forEach(child => this.MDL.highlighted.data.filter.delete(child)));
      },
    }
    ...
    updatemissingDataForCurrentFrame() {
      ...
      listItems.data().forEach(d => {
        d.missingDataForFrame = !d.missingData && _getLeafChildren(d).every(child => !currentDataMap.hasByStr(child[KEY$3]));
      });
    ...
    _interact() {
      ...
      return {
        clickToAddAllinGroup(d) {
          ...
          _this.model.data.filter.addUsingLimitedStructure({dim, isness: "is--" + oneLevelDeeper, prop, key: d[KEY$3]});
        },
        clickToRemoveAllinGroup(d) {
          const dim = _this._getPrimaryDim();
          const prop = d.prop;
          ...
          _this.model.data.filter.deleteUsingLimitedStructure({dim, isness: "is--" + oneLevelDeeper, prop, key: d[KEY$3]});
        },
        clickToRemoveAllinOtherGroups(d) {
          const dim = _this._getPrimaryDim();
          const prop = d.prop;
        ...
        disableRemoveAll(d){
          ...
          const dim = _this.model.data.space[0];
          const prop = dim;
          return _this.model.data.filter.isAlreadyRemovedUsingLimitedStructure({key: _getLeafChildren(d).map(m => m[KEY$3]), dim, prop});
        },
  ...
  const decorated$6 = mobx.decorate(SectionFind, {
    ...
    "drilldownValues": mobx.observable.struct,
    "listData": mobx.observable.struct,
    "colorScale": mobx.computed,
    "colorFields": mobx.computed,
  });
            ...
            if(!this.parent.ui.disableAddRemoveGroups) allCatalog.push(...entities
              .reduce((result, entity) => {
                ...
                if (props.length) {
                  props.forEach(prop => {
                    if (
                      !dimOrInEntityKeys.includes(this._getKey(entity[KEY$2], prop.concept, dim))
  ...
  class _MarkerControls extends decorated$e {
    constructor(config) {
      config.template = `
      <div class='vzb-dialog-modal'>
    ...
    setup(options) {
      ...
      setIcon(this.DOM.info, ICON_ELLIPSIS_V)
        .on("click", (event) => {
    ...
    updateSearch({command, arg} = this._getSearchTerm()) {
      ...
      this.sections.forEach(section => {
        section.showHideSection( !command && !arg && section.magicCommand === "find" 
  ...
  const MarkerControls = mobx.decorate(_MarkerControls, {
    "marksFromAllFrames": mobx.computed,
  ...
  class MoreOptions extends decorated$e {
    constructor(config) {
  ...
  exports.Label = Label;
  exports.LabelSizeHelper = decorated$d;
  ...
  exports.Menu = Menu;
  exports.MinMaxInputs = decorated$9;
  ...
  exports.SteppedSlider = decorated$a;
